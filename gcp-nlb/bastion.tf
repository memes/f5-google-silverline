...
# Spin up a bastion on the internal network; the bastion will be permitted to
# connect to GKE masters via an automatic VPC peer, and with BIG-IP via explicit
# peering between control and internal VPCs.
module "bastion" {
  ...
  subnet                     = lookup(lookup(module.service_vpc.subnets, format("%s/service", var.region), {}), "self_link", "")
